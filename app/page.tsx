...
export default function StudentHome() {
  ...
  const [hasIaConsent, setHasIaConsent] = useState(false); // NOVO: Estado de Consentimento
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // 2. Buscar dados do aluno (INCLUINDO CONSENTIMENTO IA)
      const { data: student } = await supabase
        .from('alunos')
        .select('nome, limite_semanal, consentimento_ia') // Puxamos a nova coluna
        .eq('id', user.id)
        .single();
      
      if (student) {
        setStudentName(student.nome?.split(' ')[0] || 'Aluno');
        setHasIaConsent(student.consentimento_ia || false); // Guardamos o estado
      ...
      <main className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center p-6 relative overflow-hidden font-sans">
        ...
        <div className="z-10 text-center space-y-8 max-w-md w-full">
          ...
          <div>
            ...
            <div className="bg-slate-900 border border-slate-800 p-6 rounded-3xl mt-4 inline-block w-full shadow-2xl relative">
          ...
          <div className="pt-4 space-y-4 w-full">
            <div className={`grid ${hasIaConsent ? 'grid-cols-3' : 'grid-cols-2'} gap-3 mb-6`}>
               ...
               {/* FILTRO LAB AI NA SESSÃO ATIVA */}
               {hasIaConsent && (
                 <Link href="/aluno/lab" className="bg-slate-900 border border-orange-500/50 p-4 rounded-2xl flex flex-col items-center justify-center gap-2 group shadow-lg shadow-orange-900/10">
                   <BrainCircuit size={24} className="text-orange-400 animate-pulse" />
            ...
            <button onClick={handleCheckout} className="w-full bg-slate-900 hover:bg-red-900/20 text-slate-400 hover:text-red-400 border border-slate-800 py-4 rounded-2xl font-black transition-all flex items-center justify-center gap-2">
        ...
        {isLimitReached ? (
          ...
          <>
            <div className="grid grid-cols-2 gap-3 mb-8">
                <Link href="/agenda" className="bg-linear-to-br from-purple-900/50 to-blue-900/50 border border-purple-500/30 p-5 rounded-3xl h-32 flex flex-col justify-between group">
                ...
                <Link href="/biblioteca" className="bg-linear-to-br from-orange-900/50 to-red-900/50 border border-orange-500/30 p-5 rounded-3xl h-32 flex flex-col justify-between group">
                ...
                {/* FILTRO LAB AI NO DASHBOARD PRINCIPAL */}
                {hasIaConsent && (
                  <Link href="/aluno/lab" className="col-span-2 bg-linear-to-r from-orange-600 to-orange-400 p-5 rounded-3xl flex items-center justify-between shadow-xl shadow-orange-900/20 hover:scale-[1.02] transition-all group">
                      <div className="flex items-center gap-4">
                        <div className="bg-white/20 p-3 rounded-2xl text-white group-hover:animate-bounce"><BrainCircuit size={28} /></div>
                        <div>
            ...
            <div className="grid grid-cols-2 gap-3 pb-10">
              {subjects.map((subject) => (
                <button key={subject.id} onClick={() => handleCheckIn(subject.id, subject.name, subject.sala_id)} className="bg-slate-900 hover:bg-slate-800 border border-slate-800 p-5 rounded-3xl text-left transition-all group relative overflow-hidden">
