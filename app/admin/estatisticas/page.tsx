...
export default function AdminStats() {
  ...
  const processStats = async () => {
    ...
    // Calcula o máximo real de forma segura
    ...
    <main className="min-h-screen bg-[#0f172a] text-white p-6 md:p-8 max-w-7xl mx-auto font-sans">
      ...
      <header className="flex items-center gap-4 mb-10">
        ...
        <div>
          <h1 className="text-3xl md:text-4xl font-black italic uppercase tracking-tighter flex items-center gap-3">
             <BarChart3 size={28} className="text-emerald-500" /> Desempenho Global
          </h1>
          <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mt-1">Monitorização de produtividade semanal</p>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        ...
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-4xl shadow-xl">
          ...
          <p className="text-[10px] text-emerald-500 font-bold mt-1 uppercase">{stats.topStudent.hours}h registadas</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-4xl shadow-xl">
           ...
           <p className="text-[10px] text-blue-500 font-bold mt-1 uppercase">{stats.topSubject.count} sessões</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 p-6 rounded-4xl shadow-xl text-red-400">
           ...
           <p className="text-[10px] font-bold mt-1 uppercase">{stats.busiestDay.count} entradas</p>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="bg-slate-900 border border-slate-800 p-8 rounded-4xl shadow-xl">
          ...
          <div className="h-64 flex items-end justify-between gap-4 px-2 border-b border-slate-800/50 pb-2">
            {stats.daysDistribution.map((d: any) => (
              <div key={d.day} className="flex flex-col items-center justify-end w-full h-full group relative">
                
                {/* Tooltip: X Entradas */}
                <div className="absolute top-0 bg-slate-800 border border-slate-700 text-white font-black text-[10px] py-1 px-3 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity z-10 shadow-xl whitespace-nowrap -mt-8">
                    {d.value} Entradas
                </div>
                
                {/* A Barra com altura forçada */}
                <div 
                    className="w-full max-w-8.75 bg-blue-600 rounded-t-xl transition-all duration-1000 ease-out group-hover:bg-blue-400 shadow-lg shadow-blue-900/20"
                    style={{ height: `${Math.max(d.percent || 0, 5)}%` }} 
                ></div>
        ...
        <div className="bg-slate-900 border border-slate-800 p-8 rounded-4xl shadow-xl">
          ...
          <div className="space-y-4">
            ...
            {stats.topStudentsList.length === 0 && <p className="text-center text-slate-600 italic text-sm py-10">A aguardar dados...</p>}
          </div>
        </div>
      </div>
    </main>
