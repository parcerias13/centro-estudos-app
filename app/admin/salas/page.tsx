...
export default function GestaoSalas() {
  ...
  return (
    <main className="min-h-screen bg-page text-primary p-6 md:p-8 max-w-6xl mx-auto font-sans">
      ...
      <header className="flex items-center gap-4 mb-10">
        ...
        <div>
          <h1 className="text-3xl md:text-4xl font-black italic uppercase tracking-tighter flex items-center gap-3">
             <MapPin size={28} className="text-accent" /> Logística e Salas
          </h1>
          <p className="text-muted text-[10px] font-black uppercase tracking-widest mt-1">Mapeamento de recursos físicos</p>
        </div>
      </header>
      ...
      <div className="grid md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-1 space-y-8">
          ...
          <section className="bg-surface border border-border rounded-3xl p-6 shadow-xl">
            <h2 className="text-sm font-black uppercase text-secondary tracking-widest mb-6 flex items-center gap-2">
              <Plus size={16} className="text-accent" /> Adicionar Sala
            </h2>
            <form onSubmit={handleCriarSala} className="space-y-4">
              <div className="space-y-2">
                <label className="text-xs font-bold text-muted uppercase tracking-widest ml-1">Nome da Sala</label>
                <input 
                  type="text" required value={novaSalaNome} onChange={(e)=>setNovaSalaNome(e.target.value)}
                  className="w-full bg-page border border-border p-4 rounded-xl outline-none focus:border-accent text-sm font-bold"
                  placeholder="Ex: Sala 1"
                />
              </div>
              <div className="space-y-2">
                <label className="text-xs font-bold text-muted uppercase tracking-widest ml-1">Lotação (Cadeiras)</label>
                <input 
                  type="number" min="1" required value={novaSalaCapacidade} onChange={(e)=>setNovaSalaCapacidade(e.target.value)}
                  className="w-full bg-page border border-border p-4 rounded-xl outline-none focus:border-accent text-sm font-bold font-mono"
                />
              </div>
              {salaError && (
                <p className="text-danger text-[11px] font-bold px-1">{salaError}</p>
              )}
              <button type="submit" disabled={savingSala} className="w-full bg-accent hover:bg-accent-hover text-on-accent font-black py-4 rounded-xl transition-all active:scale-95 flex justify-center items-center gap-2 mt-2">
                 ...
                 <div key={sala.id} className="bg-surface border border-border p-5 rounded-2xl flex items-center justify-between shadow-lg group">
                   ...
                   <div className="flex items-center gap-2">
                     <button 
                       ...
                       className="p-2.5 bg-danger-bg text-danger border border-danger/20 rounded-xl hover:bg-danger hover:text-on-danger transition-all"
                       title="Eliminar Sala"
                     >
                       <Trash2 size={18} />
                     </button>
                     <div className="bg-accent-soft text-accent p-2.5 rounded-xl border border-accent/20">
                       <MapPin size={18} />
                     </div>
                   </div>
                 </div>
        ...
        <div className="md:col-span-2">
          <section className="bg-surface border border-border rounded-3xl p-6 md:p-8 shadow-xl h-full">
            <div className="flex items-center justify-between mb-8 pb-6 border-b border-border/50">
              <h2 className="text-sm font-black uppercase text-secondary tracking-widest flex items-center gap-2">
                <BookOpen size={18} className="text-success" /> Roteamento de Disciplinas
              </h2>
              <span className="text-[10px] bg-raised text-secondary px-3 py-1 rounded-full font-black uppercase tracking-widest border border-border">
            ...
            <div className="space-y-3">
              {disciplinas.map(disc => (
                <div key={disc.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 bg-page border border-border rounded-2xl hover:border-border transition-colors shadow-inner">
                  <div>
                    <p className="font-black text-primary text-base">{disc.name}</p>
                    <p className={`text-[10px] font-black uppercase tracking-widest mt-1 ${disc.sala_id ? 'text-success' : 'text-muted'}`}>
